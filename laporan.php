...
$sql = mysqli_query($koneksi, "select a.no_nota, a.tanggal, a.nama_cust, a.biaya, b.merk, b.plat from transaksi a, mobil b where a.id_mobil=b.id_mobil and a.tanggal BETWEEN '$tgl1' AND '$tgl2'");

if(mysqli_num_rows($sql) > 0){
$no = 0;

echo '<h2>Rekap Laporan Pendapatan <small>'.$tgl1.' sampai '.$tgl2.'</small></h2><hr>
...
$sql = mysqli_query($koneksi, "SELECT count(nama_cust), sum(biaya) FROM transaksi WHERE tanggal BETWEEN '$tgl1' AND '$tgl2'");

list($nama, $biaya) = mysqli_fetch_array($sql);{
echo '<tr><td><span class="pull-right"><h4><b>'.$nama.' data</b></h4></span></td><td><span class="pull- right"><h4><b>Rp'.number_format($biaya).'</b></h4></span></td></tr>'
;
}
echo '
</table>
...
echo '<h2>Rekap Laporan Pendapatan Hari Ini (<small>'.date('d-m- Y').'</small>)</h2><hr>';
?>
...
echo '<div class="col-sm-6"><table class="table table-bordered">'; echo '<tr class="info"><th><h4>Jumlah Data</h4></th><th><h4>Jumlah Pendapatan</h4></th></tr>';

$tanggal = date('Y-m-d');

$sql = mysqli_query($koneksi, "SELECT count(nama_cust), sum(biaya) FROM transaksi WHERE tanggal='$tanggal'");

list($nama, $total) = mysqli_fetch_array($sql);{
echo '<tr><td><span class="pull-right"><h4><b>'.$nama.' data</b></h4></span></td><td><span class="pull- right"><h4><b>Rp'.number_format($total).'</b></h4></span></td></tr>'
;
}
echo '
</table>
